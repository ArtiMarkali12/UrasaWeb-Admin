...
import { diaryAPI, diaryOptionsAPI } from "../../services/api";
import "./Diary.css";
...
const Diary = () => {
  ...
  const fetchDiaries = async () => {
    try {
      ...
      showToast("Failed to fetch diary quotes", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this diary quote?")) {
      try {
        await diaryAPI.delete(id);
        setDiaries(diaries.filter((d) => d._id !== id));
        showToast("Diary quote deleted successfully", "success");
      } catch (error) {
        console.error("Error deleting diary:", error);
        showToast("Failed to delete diary quote", "error");
      }
    }
  };
  ...
  const handleEdit = (diary) => {
    ...
    setFormData({
      ...
      orderDate: diary.timeline?.orderDate
        ? new Date(diary.timeline.orderDate).toISOString().split("T")[0]
        : "",
      expectedDate: diary.timeline?.expectedDate
        ? new Date(diary.timeline.expectedDate).toISOString().split("T")[0]
        : "",
      deliveryDate: diary.timeline?.deliveryDate
        ? new Date(diary.timeline.deliveryDate).toISOString().split("T")[0]
        : "",
    });
    setSelectedDiary(diary);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      await diaryAPI.update(selectedDiary._id, updateData);
      fetchDiaries();
      setShowEditModal(false);
      document.body.classList.remove("modal-open");
      setSelectedDiary(null);
      showToast("Diary quote updated successfully", "success");
    } catch (error) {
      console.error("Error updating diary:", error);
      showToast("Failed to update diary quote", "error");
    }
  };

  const filteredDiaries = diaries.filter(
    (diary) =>
      diary.customerDetails?.name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      diary.customerDetails?.email
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()),
  );
  ...
  const fetchOptions = async () => {
    try {
      const response = await diaryOptionsAPI.getAll();
  ...
  const fetchDropdownOptions = async () => {
    try {
      const response = await diaryOptionsAPI.getDropdown();
  ...
  const handleAddAttribute = async (categoryKey, subcategoryKey) => {
    ...
    try {
      const response = await diaryOptionsAPI.addAttribute(
        categoryKey,
        subcategoryKey,
        { value: value },
      );
  ...
  const handleUpdateAttribute = async (e) => {
    ...
    try {
      ...
      const response = await diaryOptionsAPI.updateAttribute(
        updateCategory,
        updateSubcategory,
        index,
  ...
  const handleDeleteAttribute = async (value) => {
    ...
    try {
      ...
      const response = await diaryOptionsAPI.deleteAttribute(
        selectedCategory,
        selectedSubcategory,
        index,
      );
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await diaryOptionsAPI.addCategory({
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      const response = await diaryOptionsAPI.updateCategory(editingCategory, {
  ...
  const handleAddSubcategory = async (e) => {
    ...
    try {
      const response = await diaryOptionsAPI.addSubcategory(selectedCategory, {
  ...
  const handleDeleteSubcategory = async (subcategoryKey, subcategoryLabel) => {
    if (
      ...
      return;
    try {
      await diaryOptionsAPI.deleteSubcategory(selectedCategory, subcategoryKey);
  ...
  const handleUpdateSubcategory = async (e) => {
    ...
    try {
      const response = await diaryOptionsAPI.updateSubcategory(
        editingCategory,
        editingSubcategory,
        {
        ...
        <div className="tab-content">
          ...
          {loading ? (
            <div className="loading-container">
              ...
              <p>Loading diary quotes...</p>
            </div>
          ) : (
            <div className="booklets-grid">
              {filteredDiaries.length > 0 ? (
                filteredDiaries.map((diary) => (
                  <div key={diary._id} className="booklet-card">
                    <div className="card-badge">
                      #{diary._id.slice(-6).toUpperCase()}
                    </div>
                    <div className="card-header">
                      <div className="customer-avatar">
                        {diary.customerDetails?.name?.charAt(0) || "C"}
                      </div>
                              ...
                              <div className="category-attributes-direct-section">
                                ...
                                <form
                                  ...
                                  onSubmit={async (e) => {
                                    ...
                                    try {
                                      const response =
                                        await diaryOptionsAPI.addCategoryAttribute(
                                          categoryKey,
                                          { value: value },
                                        );
                                      setAttributeInputs((prev) => ({
                                        ...
                                        >
                                          {isEditing ? (
                                            <form
                                              ...
                                              onSubmit={async (e) => {
                                                e.preventDefault();
                                                await diaryOptionsAPI.updateCategoryAttribute(
                                                  categoryKey,
                                                  index,
                                                  { value: editOptionValue },
                                                );
                                              ...
                                              <div className="chip-actions">
                                                ...
                                                <button
                                                  ...
                                                  onClick={async () => {
                                                    ...
                                                    await diaryOptionsAPI.deleteCategoryAttribute(
                                                      categoryKey,
                                                      index,
                                                    );
                                                    fetchOptions();
        ...
        <div className="tab-content">
          <div className="view-options-header">
            <div>
              ...
              <p>
                View all diary configuration options organized by categories
              </p>
            </div>
          </div>
          {optionsLoading ? (
        ...
        >
          ...
          >
            <div className="simple-modal-header">
              ...
              <p>Create a new category to manage diary options</p>
            </div>
            <form
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <h2>Diary Quote Details</h2>
            </div>
            <div className="modal-body">
              {renderDynamicData(selectedDiary)}
            </div>
          </div>
        </div>
      )}
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group checkbox-group">
                      <label>
                        <input
                          type="checkbox"
                          checked={formData.coverFlaps || false}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
