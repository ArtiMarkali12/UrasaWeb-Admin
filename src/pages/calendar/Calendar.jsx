...
import { calendarAPI, calendarOptionsAPI } from "../../services/api";
import "../booklet/Booklet.css"; // Using same CSS file as requested
...
const Calendars = () => {
  ...
  const fetchCalendars = async () => {
    try {
      ...
      showToast("Failed to fetch calendar quotes", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this calendar quote?")) {
      try {
        await calendarAPI.delete(id);
        setCalendars(calendars.filter((c) => c._id !== id));
        showToast("Calendar quote deleted successfully", "success");
      } catch (error) {
        console.error("Error deleting calendar:", error);
        showToast("Failed to delete calendar quote", "error");
      }
    }
  };
  ...
  const handleEdit = (calendar) => {
    ...
    setFormData({
      ...formDataDynamic,
      orderType: calendar.orderType || "",
      customerName: calendar.customerDetails?.name || "",
      customerEmail: calendar.customerDetails?.email || "",
      customerCountry: calendar.customerDetails?.country || "",
      orderDate: calendar.timeline?.orderDate
        ? new Date(calendar.timeline.orderDate).toISOString().split("T")[0]
        : "",
      expectedDate: calendar.timeline?.expectedDate
        ? new Date(calendar.timeline.expectedDate).toISOString().split("T")[0]
        : "",
      deliveryDate: calendar.timeline?.deliveryDate
        ? new Date(calendar.timeline.deliveryDate).toISOString().split("T")[0]
        : "",
    });
    setSelectedCalendar(calendar);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (dropdownOptions && dropdownOptions.length > 0) {
        ...
        Object.assign(options, selectedCalendar.options || {});
      }
      ...
      await calendarAPI.update(selectedCalendar._id, updateData);
      fetchCalendars();
      setShowEditModal(false);
      document.body.classList.remove("modal-open");
      setSelectedCalendar(null);
      showToast("Calendar quote updated successfully", "success");
    } catch (error) {
      console.error("Error updating calendar:", error);
      showToast("Failed to update calendar quote", "error");
    }
  };

  const filteredCalendars = calendars.filter(
    (calendar) =>
      calendar.customerDetails?.name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      calendar.customerDetails?.email
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()),
  );
  ...
  const fetchOptions = async () => {
    try {
      const response = await calendarOptionsAPI.getAll();
  ...
  const fetchDropdownOptions = async () => {
    try {
      const response = await calendarOptionsAPI.getDropdown();
  ...
  const handleAddAttribute = async (categoryKey, subcategoryKey) => {
    ...
    try {
      const response = await calendarOptionsAPI.addAttribute(
        categoryKey,
        subcategoryKey,
        { value: value },
      );
  ...
  const handleUpdateAttribute = async (e) => {
    ...
    try {
      ...
      const response = await calendarOptionsAPI.updateAttribute(
        updateCategory,
        updateSubcategory,
        index,
  ...
  const handleDeleteAttribute = async (value) => {
    ...
    try {
      ...
      const response = await calendarOptionsAPI.deleteAttribute(
        selectedCategory,
        selectedSubcategory,
        index,
      );
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await calendarOptionsAPI.addCategory({
  ...
  const handleDeleteCategory = async (categoryKey, categoryLabel) => {
    if (
      ...
      return;
    try {
      await calendarOptionsAPI.deleteCategory({ categoryKey });
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      const response = await calendarOptionsAPI.updateCategory(editingCategory, {
  ...
  const handleAddSubcategory = async (e) => {
    ...
    try {
      const response = await calendarOptionsAPI.addSubcategory(
        selectedCategory,
        {
  ...
  const handleDeleteSubcategory = async (subcategoryKey, subcategoryLabel) => {
    if (
      ...
      return;
    try {
      await calendarOptionsAPI.deleteSubcategory(
        selectedCategory,
        subcategoryKey,
      );
  ...
  const handleUpdateSubcategory = async (e) => {
    ...
    try {
      const response = await calendarOptionsAPI.updateSubcategory(
        editingCategory,
        editingSubcategory,
        {
  ...
  const renderDynamicData = (data, parentKey = "") => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      // Handle nested objects - FULLY DYNAMIC
      if (value && typeof value === "object" && !Array.isArray(value)) {
        // For options object, render all children as flat info-items
        if (key === "options") {
  ...
  return (
    <div className="booklets-page"> {/* Using same CSS classes */}
      {toast.show && (
        ...
        <div className="tab-content">
          ...
          {loading ? (
            <div className="loading-container">
              ...
              <p>Loading calendar quotes...</p>
            </div>
          ) : (
            <div className="booklets-grid">
              {filteredCalendars.length > 0 ? (
                filteredCalendars.map((calendar) => (
                  <div key={calendar._id} className="booklet-card">
                    <div className="card-badge">
                      #{calendar._id.slice(-6).toUpperCase()}
                    </div>
                    <div className="card-header">
                      <div className="customer-avatar">
                        {calendar.customerDetails?.name?.charAt(0) || "C"}
                      </div>
                    ...
                    <div className="card-body">
                      ...
                      <div className="info-row">
                        ...
                        <span className="info-value">
                          {calendar.orderType || "N/A"}
                        </span>
                      </div>

                      {/* Dynamically render ALL fields from options */}
                      {calendar.options &&
                        typeof calendar.options === "object" &&
                        Object.entries(calendar.options).map(
                              ...
                              <div className="category-attributes-direct-section">
                                ...
                                <form
                                  ...
                                  onSubmit={async (e) => {
                                    ...
                                    try {
                                      const response =
                                        await calendarOptionsAPI.addCategoryAttribute(
                                          categoryKey,
                                          { value: value },
                                        );
                                      setAttributeInputs((prev) => ({
                                        ...
                                        >
                                          {isEditing ? (
                                            <form
                                              ...
                                              onSubmit={async (e) => {
                                                e.preventDefault();
                                                await calendarOptionsAPI.updateCategoryAttribute(
                                                  categoryKey,
                                                  index,
                                                  { value: editOptionValue },
                                                );
                                              ...
                                              <div className="chip-actions">
                                                ...
                                                <button
                                                  ...
                                                  onClick={async () => {
                                                    ...
                                                    await calendarOptionsAPI.deleteCategoryAttribute(
                                                      categoryKey,
                                                      index,
                                                    );
                                                    fetchOptions();
        ...
        <div className="tab-content">
          <div className="view-options-header">
            <div>
              ...
              <p>
                View all calendar configuration options organized by categories
              </p>
            </div>
          </div>
          {optionsLoading ? (
        ...
        >
          ...
          >
            <div className="simple-modal-header">
              ...
              <p>Create a new category to manage calendar options</p>
            </div>
            <form
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <h2>Calendar Quote Details</h2>
            </div>
            <div className="modal-body">
              {renderDynamicData(selectedCalendar)}
            </div>
          </div>
        </div>
      )}
          ...
          >
            ...
            <form className="edit-form" onSubmit={handleEditSubmit}>
              <div className="modal-body">
                <div className="modal-section">
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group">
                      ...
                      <input
                        type="text"
                        value={formData.calendarSize}
                        onChange={(e) =>
                          setFormData({ ...formData, calendarSize: e.target.value })
                        }
                        required
                      />
                    </div>
                    <div className="form-group">
                      ...
                      >
                        ...
                        <option value="Magnetic Calendar">Magnetic Calendar</option>
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group checkbox-group">
                      <label>
                        <input
                          type="checkbox"
                          checked={formData.wireOBinding}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              wireOBinding: e.target.checked,
                            })
                          }
                        />{" "}
                        Wire-O Binding
                ...
                <div className="modal-section">
                  <div className="section-icon">📄</div>
                  <h3>Paper & Print Specifications</h3>
                  <div className="form-grid">
                    <div className="form-group">
