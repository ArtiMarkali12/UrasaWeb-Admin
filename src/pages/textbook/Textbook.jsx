...
const Textbook = () => {
  ...
  const fetchTextbooks = async () => {
    try {
      ...
      showToast("Failed to fetch textbook quotes", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this textbook quote?")) {
      try {
        await textbookAPI.delete(id);
        setTextbooks(textbooks.filter((b) => b._id !== id));
        showToast("Textbook quote deleted successfully", "success");
      } catch (error) {
        console.error("Error deleting textbook:", error);
        showToast("Failed to delete textbook quote", "error");
      }
    }
  };

  const handleView = (textbook) => {
  ...
  const handleEdit = (textbook) => {
    const opts = textbook.options || {};
    const formDataDynamic = {};
    Object.entries(opts).forEach(([catKey, catData]) => {
    ...
    setFormData({
      ...formDataDynamic,
      orderType: textbook.orderType || "",
      customerName: textbook.customerDetails?.name || "",
      customerEmail: textbook.customerDetails?.email || "",
      customerCountry: textbook.customerDetails?.country || "",
      orderDate: textbook.timeline?.orderDate
        ? new Date(textbook.timeline.orderDate).toISOString().split("T")[0]
        : "",
      expectedDate: textbook.timeline?.expectedDate
        ? new Date(textbook.timeline.expectedDate).toISOString().split("T")[0]
        : "",
      deliveryDate: textbook.timeline?.deliveryDate
        ? new Date(textbook.timeline.deliveryDate).toISOString().split("T")[0]
        : "",
    });
    setSelectedTextbook(textbook);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
          ...
          if (subcategories.length > 0) {
            ...
            subcategories.forEach((subcat) => {
              ...
              if (formData[subKey] !== undefined) {
                opts[catKey][subKey] = formData[subKey];
              }
            });
          } else {
            if (formData[catKey] !== undefined) {
              opts[catKey] = formData[catKey];
            }
          }
        });
      } else {
        Object.assign(opts, selectedTextbook.options || {});
      }
      ...
      await textbookAPI.update(selectedTextbook._id, updateData);
      fetchTextbooks();
      setShowEditModal(false);
      document.body.classList.remove("modal-open");
      setSelectedTextbook(null);
      showToast("Textbook quote updated successfully", "success");
    } catch (error) {
      console.error("Error updating textbook:", error);
      showToast("Failed to update textbook quote", "error");
    }
  };

  const filteredTextbooks = textbooks.filter(
    (textbook) =>
      textbook.customerDetails?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      textbook.customerDetails?.email?.toLowerCase().includes(searchTerm.toLowerCase()),
  );

  const fetchOptions = async () => {
    try {
      const response = await textbookOptionsAPI.getAll();
  ...
  const fetchDropdownOptions = async () => {
    try {
      const response = await textbookOptionsAPI.getDropdown();
  ...
  const handleAddAttribute = async (categoryKey, subcategoryKey) => {
    ...
    try {
      const response = await textbookOptionsAPI.addAttribute(categoryKey, subcategoryKey, { value });
  ...
  const handleUpdateAttribute = async (e) => {
    ...
    try {
      ...
      const response = await textbookOptionsAPI.updateAttribute(
        updateCategory,
        updateSubcategory,
        index,
  ...
  const handleDeleteAttribute = async (value) => {
    ...
    try {
      ...
      const response = await textbookOptionsAPI.deleteAttribute(selectedCategory, selectedSubcategory, index);
      fetchOptions();
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await textbookOptionsAPI.addCategory({
      ...
      fetchOptions();
      showToast(
        response?.data?.message || `Category "${newCategoryName}" added successfully`,
        "success",
      );
    } catch (error) {
  ...
  const handleDeleteCategory = async (categoryKey, categoryLabel) => {
    ...
    try {
      await textbookOptionsAPI.deleteCategory(categoryKey);
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      const response = await textbookOptionsAPI.updateCategory(editingCategory, {
  ...
  const handleAddSubcategory = async (e) => {
    ...
    try {
      const response = await textbookOptionsAPI.addSubcategory(selectedCategory, {
      ...
      fetchOptions();
      showToast(
        response?.data?.message || `Subcategory "${newSubcategoryName}" added successfully`,
        "success",
      );
    } catch (error) {
  ...
  const handleDeleteSubcategory = async (subcategoryKey, subcategoryLabel) => {
    ...
    try {
      await textbookOptionsAPI.deleteSubcategory(selectedCategory, subcategoryKey);
  ...
  const handleUpdateSubcategory = async (e) => {
    ...
    try {
      const response = await textbookOptionsAPI.updateSubcategory(
        editingCategory,
        editingSubcategory,
        {
        ...
        if (key === "options") {
          Object.entries(value).forEach(([optKey, optValue]) => {
            ...
            if (optKey === "sizeSelection" && optValue && typeof optValue === "object") {
              const sizeVal =
                optValue.selectedSize || optValue.cardSize || optValue.dimensions || "";
              if (sizeVal) {
                items.push(
                  ...
                  <div key={textbook._id} className="textbook-card">
                    ...
                    <div className="card-header">
                      <div className="customer-avatar">
                        {textbook.customerDetails?.name?.charAt(0) || "C"}
                      </div>
                      <div className="customer-name">{textbook.customerDetails?.name}</div>
                    </div>
                    <div className="card-body">
                      <div className="info-row">
                        <span className="info-label">Email</span>
                        <span className="info-value">{textbook.customerDetails?.email}</span>
                      </div>
                      <div className="info-row">
                        <span className="info-label">Order Type</span>
                        <span className="info-value">{textbook.orderType || "N/A"}</span>
                      </div>
                      {textbook.options?.sizeSelection && (
                        <div className="info-row">
                          <span className="info-label">Size</span>
                          <span className="info-value">
                            {typeof textbook.options.sizeSelection === "string"
                              ? textbook.options.sizeSelection
                              : textbook.options.sizeSelection.selectedSize ||
                                textbook.options.sizeSelection.cardSize ||
                                "N/A"}
                          </span>
                        </div>
                      )}
                      {textbook.options &&
                        typeof textbook.options === "object" &&
                        Object.entries(textbook.options).map(([categoryKey, categoryData]) => {
                          if (
                            !categoryData ||
                    ...
                    <div className="card-footer">
                      <div className="card-date">{formatDate(textbook.createdAt)}</div>
                      <div className="card-actions">
                        <button className="action-btn view-btn" onClick={() => handleView(textbook)}>
                          View
                        </button>
                        <button className="action-btn edit-btn" onClick={() => handleEdit(textbook)}>
                          Edit
                        </button>
                        <button
                  ...
                  <div key={categoryKey} className="unified-category-card">
                    ...
                    >
                      ...
                      <div className="category-header-actions">
                        ...
                        >
                          <span style={{ color: "white", fontWeight: "bold" }}>+</span> Add
                          Subcategory
                        </button>
                        <button
                        ...
                        {hasSubcategories ? (
                          <div className="subcategories-section">
                              ...
                              <div className="category-attributes-direct-section">
                                ...
                                <form
                                  ...
                                  onSubmit={async (e) => {
                                    ...
                                    try {
                                      const response =
                                        await textbookOptionsAPI.addCategoryAttribute(categoryKey, {
                                          value,
                                        });
                                      setAttributeInputs((prev) => ({
                                        ...prev,
                                ...
                                >
                                  <input
                                    ...
                                    value={attributeInputs[`${categoryKey}-category`] || ""}
                                    onChange={(e) =>
                                      setAttributeInputs((prev) => ({
                                        ...prev,
                                  ...
                                  />
                                  <button type="submit" className="add-attribute-btn">
                                    ➕ Add
                                  </button>
                                </form>
                                <div className="attributes-chip-list">
                                  {categoryAttributes.length > 0 ? (
                                    categoryAttributes.map((attr, index) => {
                                      const isEditing =
                                        editingOption === `${categoryKey}-category-${index}`;
                                      return (
                                        <div key={attr} className="attribute-chip">
                                          {isEditing ? (
                                            <form
                                              ...
                                              onSubmit={async (e) => {
                                                e.preventDefault();
                                                await textbookOptionsAPI.updateCategoryAttribute(
                                                  categoryKey,
                                                  index,
                                                  { value: editOptionValue },
                                                );
                                            ...
                                            <>
                                              ...
                                              <div className="chip-actions">
                                                ...
                                                <button
                                                  ...
                                                  onClick={async () => {
                                                    if (!window.confirm(`Delete "${attr}"?`)) return;
                                                    setSelectedCategory(categoryKey);
                                                    await textbookOptionsAPI.deleteCategoryAttribute(
                                                      categoryKey,
                                                      index,
                                                    );
                                                    fetchOptions();
                            ...
                            <div
                              ...
                              style={{ marginTop: categoryAttributes.length > 0 ? "24px" : "0" }}
                            >
                              <h4>Subcategories</h4>
                              <span className="subsection-info">
                                  ...
                                  <div key={subcatKey} className="unified-subcategory-card">
                                    <div className="unified-subcategory-header">
                                      <div className="subcategory-header-content">
                                        <span className="subcategory-name">
                                          {subcategory?.displayName || formatLabel(subcatKey)}
                                        </span>
                                      </div>
                                      <div className="subcategory-header-actions">
                                        ...
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            handleEditSubcategory(subcatKey, subcategory, categoryKey);
                                          }}
                                    ...
                                    <div className="attributes-section">
                                      <form
                                        ...
                                        onSubmit={async (e) => {
                                          e.preventDefault();
                                          await handleAddAttribute(categoryKey, subcatKey);
                                        }}
                                      >
                                        <input
                                          type="text"
                                          placeholder="Enter attribute value..."
                                          value={
                                            attributeInputs[`${categoryKey}-${subcatKey}`] || ""
                                          }
                                          onChange={(e) =>
                                            setAttributeInputs((prev) => ({
                                              ...prev,
                                              [`${categoryKey}-${subcatKey}`]: e.target.value,
                                            }))
                                          }
                                          className="attribute-input"
                                        />
                          ...
                          <div className="attributes-only-section">
                            ...
                            <p className="prompt-subtext" style={{ marginTop: "12px" }}>
                              💡 Tip: You can also add more subcategories to organize attributes
                              better
                            </p>
                            <button
        ...
        <div className="tab-content">
          <div className="view-options-header">
            <div>
              ...
              <p>View all textbook configuration options organized by categories</p>
            </div>
          </div>
          {optionsLoading ? (
            <div className="loading-container">
                        ...
                        {categoryAttributes.length > 0 && (
                          <div className="hierarchical-category-attributes-section">
                            ...
                            <div className="hierarchical-attributes-list">
                              {categoryAttributes.map((attr, index) => (
                                <span key={index} className="hierarchical-attribute-tag">
                                  {attr}
                                </span>
                              ))}
                            </div>
        ...
        >
          ...
          >
            <div className="simple-modal-header">
              ...
              <p>Create a new category to manage textbook options</p>
            </div>
            <form
              ...
              <div className="simple-form-body">
                ...
                <div className="simple-form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewSubcategoryPlaceholder(e.target.value)}
                    placeholder="e.g., Enter value..."
                  />
                  <small>Optional placeholder text</small>
                </div>
              ...
              <div className="simple-form-body">
                ...
                <div className="simple-form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setEditSubcategoryFieldType(e.target.value)}
                    className="simple-form-select"
                  >
                    <option value="select">Dropdown Select</option>
                    <option value="checkbox">Checkbox (Multiple Options)</option>
                ...
                <div className="simple-form-group">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditSubcategoryPlaceholder(e.target.value)}
                    placeholder="e.g., Enter value..."
                  />
                  <small>Optional placeholder text</small>
                </div>
                <div className="simple-form-group">
                  <label className="checkbox-label">
                    <input
                      ...
                      onChange={(e) => setEditSubcategoryRequired(e.target.checked)}
                    />
                    <span>Required Field</span>
                  </label>
                </div>
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <h2>Textbook Quote Details</h2>
            </div>
            <div className="modal-body">{renderDynamicData(selectedTextbook)}</div>
          </div>
        </div>
      )}
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group">
                      ...
                      >
                        ...
                        {selectedCategoryDropdown &&
                          dropdownOptions
                            .find((opt) => opt.categoryKey === selectedCategoryDropdown)
                    ...
                    <div className="form-group">
                      ...
                      <select
                        ...
                        onChange={(e) => setSelectedAttributeDropdown(e.target.value)}
                      ...
                      >
                        ...
                        {selectedSubcategoryDropdown &&
                          dropdownOptions
                            .find((opt) => opt.categoryKey === selectedCategoryDropdown)
                            ?.subcategories.find(
                              (sub) => sub.subcategoryKey === selectedSubcategoryDropdown,
                            )
                            ?.attributes.map((attr) => (
