...
import { postcardAPI, postcardOptionsAPI } from "../../services/api";
import "./Postcard.css";
...
const Postcard = () => {
  ...
  const fetchPostcards = async () => {
    try {
      ...
      showToast("Failed to fetch postcard quotes", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Are you sure you want to delete this postcard quote?")
    ) {
      try {
        await postcardAPI.delete(id);
        setPostcards(postcards.filter((p) => p._id !== id));
        showToast("Postcard quote deleted successfully", "success");
      } catch (error) {
        console.error("Error deleting postcard:", error);
        showToast("Failed to delete postcard quote", "error");
      }
    }
  };

  const handleView = (postcard) => {
  ...
  const handleEdit = (postcard) => {
    ...
    const options = postcard.options || {};
    ...
    setFormData({
      ...formDataDynamic,
      orderType: postcard.orderType || "",
      customerName: postcard.customerDetails?.name || "",
      customerEmail: postcard.customerDetails?.email || "",
      customerCountry: postcard.customerDetails?.country || "",
      orderDate: postcard.timeline?.orderDate
        ? new Date(postcard.timeline.orderDate).toISOString().split("T")[0]
        : "",
      expectedDate: postcard.timeline?.expectedDate
        ? new Date(postcard.timeline.expectedDate).toISOString().split("T")[0]
        : "",
      deliveryDate: postcard.timeline?.deliveryDate
        ? new Date(postcard.timeline.deliveryDate).toISOString().split("T")[0]
        : "",
    });
    setSelectedPostcard(postcard);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (dropdownOptions && dropdownOptions.length > 0) {
        ...
        // Fallback: use existing options from selectedPostcard
        Object.assign(options, selectedPostcard.options || {});
      }
      ...
      await postcardAPI.update(selectedPostcard._id, updateData);
      fetchPostcards();
      setShowEditModal(false);
      document.body.classList.remove("modal-open");
      setSelectedPostcard(null);
      showToast("Postcard quote updated successfully", "success");
    } catch (error) {
      console.error("Error updating postcard:", error);
      showToast("Failed to update postcard quote", "error");
    }
  };

  const filteredPostcards = postcards.filter(
    (postcard) =>
      postcard.customerDetails?.name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      postcard.customerDetails?.email
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()),
  );
  ...
  const fetchOptions = async () => {
    try {
      const response = await postcardOptionsAPI.getAll();
  ...
  const fetchDropdownOptions = async () => {
    try {
      const response = await postcardOptionsAPI.getDropdown();
  ...
  const handleAddAttribute = async (categoryKey, subcategoryKey) => {
    ...
    try {
      const response = await postcardOptionsAPI.addAttribute(
        categoryKey,
        subcategoryKey,
        { value: value },
      );
  ...
  const handleUpdateAttribute = async (e) => {
    ...
    try {
      ...
      const response = await postcardOptionsAPI.updateAttribute(
        updateCategory,
        updateSubcategory,
        index,
  ...
  const handleDeleteAttribute = async (value) => {
    ...
    try {
      ...
      const response = await postcardOptionsAPI.deleteAttribute(
        selectedCategory,
        selectedSubcategory,
        index,
      );
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      const response = await postcardOptionsAPI.addCategory({
  ...
  const handleUpdateCategory = async (e) => {
    ...
    try {
      const response = await postcardOptionsAPI.updateCategory(
        editingCategory,
        {
  ...
  const handleAddSubcategory = async (e) => {
    ...
    try {
      const response = await postcardOptionsAPI.addSubcategory(
        selectedCategory,
        {
  ...
  const handleDeleteSubcategory = async (subcategoryKey, subcategoryLabel) => {
    if (
      ...
      return;
    try {
      await postcardOptionsAPI.deleteSubcategory(
        selectedCategory,
        subcategoryKey,
      );
  ...
  const handleUpdateSubcategory = async (e) => {
    ...
    try {
      const response = await postcardOptionsAPI.updateSubcategory(
        editingCategory,
        editingSubcategory,
        {
        ...
        <div className="tab-content">
          ...
          {loading ? (
            <div className="loading-container">
              ...
              <p>Loading postcard quotes...</p>
            </div>
          ) : (
            <div className="booklets-grid">
              {filteredPostcards.length > 0 ? (
                filteredPostcards.map((postcard) => (
                  <div key={postcard._id} className="booklet-card">
                    <div className="card-badge">
                      #{postcard._id.slice(-6).toUpperCase()}
                    </div>
                    <div className="card-header">
                      <div className="customer-avatar">
                        {postcard.customerDetails?.name?.charAt(0) || "P"}
                      </div>
                    ...
                    <div className="card-footer">
                      ...
                      <div className="card-actions">
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(postcard._id)}
                        >
                          Delete
                        </button>
                      </div>
                              ...
                              <div className="category-attributes-direct-section">
                                ...
                                <form
                                  ...
                                  onSubmit={async (e) => {
                                    ...
                                    try {
                                      const response =
                                        await postcardOptionsAPI.addCategoryAttribute(
                                          categoryKey,
                                          { value: value },
                                        );
                                      setAttributeInputs((prev) => ({
                                        ...
                                        >
                                          {isEditing ? (
                                            <form
                                              ...
                                              onSubmit={async (e) => {
                                                e.preventDefault();
                                                await postcardOptionsAPI.updateCategoryAttribute(
                                                  categoryKey,
                                                  index,
                                                  { value: editOptionValue },
                                                );
                                              ...
                                              <div className="chip-actions">
                                                ...
                                                <button
                                                  ...
                                                  onClick={async () => {
                                                    ...
                                                    await postcardOptionsAPI.deleteCategoryAttribute(
                                                      categoryKey,
                                                      index,
                                                    );
                                                    fetchOptions();
        ...
        <div className="tab-content">
          <div className="view-options-header">
            <div>
              ...
              <p>
                View all postcard configuration options organized by categories
              </p>
            </div>
          </div>
          {optionsLoading ? (
        ...
        >
          ...
          >
            <div className="simple-modal-header">
              ...
              <p>Category Key will be used internally for identification</p>
            </div>
              ...
              <div className="simple-form-body">
                <div className="simple-form-group">
                  ...
                  <input
                    ...
                    placeholder="e.g., paperType, bindingStyle"
                    required
                  />
        ...
        >
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <h2>Postcard Quote Details</h2>
            </div>
            <div className="modal-body">
              {/* Dynamically render ALL data from database */}
              {renderDynamicData(selectedPostcard)}
            </div>
          </div>
        </div>
      )}

      {showEditModal && selectedPostcard && (
        <div
          className="modal-overlay"
          onClick={() => {
            setShowEditModal(false);
                            ...
                            return (
                              <div className="form-group" key={subKey}>
                                ...
                                <input
                                  ...
                                  placeholder={
                                    subcat.placeholder || displayName
                                  }
                                />
                              </div>
                            );
